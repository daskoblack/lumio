...
Flux MVP :
  lectio analyze cours.pdf         -> crée le job, affiche le plan de sections
  lectio set-duration <job> -s 2 -d 60s   -> fixe une durée cible (optionnel)
  lectio script <job>              -> génère les narrations (durées contraintes)
  lectio synthesize <job>          -> TTS + durée réelle + calibration + correction
  lectio render <job>              -> slides + timeline + montage FFmpeg -> MP4
...
def _print_plan(course: Course) -> None:
    ...
    console.print(
        f"\n[bold]{course.title}[/]  "
        f"[dim](job {course.id} · état {course.status.value})[/]"
    )
    if course.truncated:
        console.print(
    ...
    for s in course.sections:
        ...
        words = ""
        if s.script:
            words = str(s.script.word_count_actual)
            if s.script.generation_pass == 2:
                words += " *"  # * = corrigé en 2e passe
        table.add_row(
            str(s.index),
            s.kind.value,
            s.title,
    ...
    if any(s.script and s.script.generation_pass == 2 for s in course.sections):
        console.print("[dim]* narration ajustée en 2e passe pour tenir la cible.[/]")
    for s in course.sections:
        if s.synthesis_note:
...
def analyze(
    ...
    orch = _orchestrator()
    try:
        course = asyncio.run(orch.create_and_analyze(pdf, title))
...
def script(job: str = typer.Argument(..., help="Identifiant du job.")) -> None:
    """Génère les narrations 'professeur' en respectant les durées cibles."""
    orch = _orchestrator()
    console.print("[dim]Génération des scripts…[/]")
    try:
...
def synthesize(job: str = typer.Argument(..., help="Identifiant du job.")) -> None:
    """Synthétise l'audio (TTS), mesure la durée réelle, calibre et corrige si besoin."""
    orch = _orchestrator()
...
def render(job: str = typer.Argument(..., help="Identifiant du job.")) -> None:
    """Rend les slides, construit la timeline et assemble la vidéo (FFmpeg)."""
    orch = _orchestrator()
    console.print("[dim]Rendu des slides et montage vidéo…[/]")
    try:
...
def show_script(
    ...
    """Affiche le texte de narration généré pour une section."""
    orch = _orchestrator()
    try:
        course = orch.store.load(job)
    except LectioError as exc:
    ...
    if sec is None or sec.script is None:
        _fail("Aucun script pour cette section (as-tu lancé 'script' ?).")
    console.print(f"[bold]{sec.title}[/] ({sec.script.word_count_actual} mots)\n")
    console.print(sec.script.text)
