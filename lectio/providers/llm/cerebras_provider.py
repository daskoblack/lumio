...
Alternative à Groq : tier gratuit à 1 000 000 tokens/jour (contre 100 000
chez Groq) — 10x plus de marge avant de heurter un plafond quotidien.
Même stratégie d'espacement + retry unique sur 429 que GroqLLM.
"""
...
class CerebrasLLM(LLMProvider):
    ...
    def __init__(
        ...
        if not api_key:
            raise LLMError(
                "Clé API Cerebras absente. Définis la variable CEREBRAS_API_KEY."
            )
        ...
        *,
        ...
        try:
            ...
            if "429" not in str(exc) or not retry_after:
                raise LLMError(f"Appel Cerebras échoué : {exc}") from exc
            ...
            try:
                ...
                raise LLMError(f"Appel Cerebras échoué (après retry) : {exc2}") from exc2
