...
class Slide(BaseModel):
    """Une slide source (une par page PDF au MVP). Le rendu viendra en phase 4."""
...
class Script(BaseModel):
    """Narration 'professeur' d'une section (texte original, pas une lecture du PDF)."""
...
class Section(BaseModel):
    """Section pédagogique. Porte la durée cible OPTIONNELLE de l'utilisateur."""
    ...
    # --- Durées ---
    estimated_narration_words: int = 0      # estimé par le LLM à l'analyse
    target_duration_s: float | None = None  # INPUT UTILISATEUR optionnel (None = auto)
    estimated_duration_s: float = 0.0        # mots estimés / débit (avant TTS)
