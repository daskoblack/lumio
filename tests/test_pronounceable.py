...
from lectio.pipeline.scripting import generate_slide_script
...
def _section():
    return Section(index=0, kind=SectionKind.CONCEPT, title="Partie 1")
...
async def test_reponse_imprononcable_declenche_une_seconde_tentative():
    ...
    script = await generate_slide_script(
        llm, _section(), _slide(), 1, 1, None, None, None, 0.10, 2
    )
...
async def test_secours_ultime_si_la_page_est_vide():
    ...
    llm = _LLMScripted(["...", "..."])
    script = await generate_slide_script(llm, section, slide, 1, 1, None, None, None, 0.10, 2)
...
async def test_correction_imprononcable_est_ignoree():
    ...
    script = await generate_slide_script(
        llm, _section(), _slide(), 1, 1, None, None, target_words=200, tolerance=0.10, max_passes=2
    )
